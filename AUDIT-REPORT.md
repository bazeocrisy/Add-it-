...
**Final carry / regroup destinations (§10–§12):** regroup values render above their **destination** place from engine `carryPlace` metadata; the leftmost carry-out is excluded from the regroup row because it *is* the leading answer digit — verified for 59+55 (regroup `1` over TENS only; HUNDREDS shows answer digit `1` once), 589+476, and 9999+9999: leading track always has empty regroup/addend cells and exactly one answer digit. All four transitions (ones→tens, tens→hundreds, hundreds→thousands, thousands→ten-thousands) exercised. No-regroup problems show fully blank regroup rows — zero fake `0` carries (§11).
